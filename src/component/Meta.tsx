...
import { drawingSchemaIdAtom, getActionSchemaTypeAtom } from '../store/schema';
...
export const Meta = () => {
  ...
  return (
    <div className='fixed top-0 left-0 m-4' key={selectedSchemaId} onWheel={(e) => e.stopPropagation()}>
      <Card className='max-h-[90vh] overflow-auto p-4 w-[340px]'>
              ...
              <ButtonGroup isIconOnly className='justify-start' variant='bordered' size='sm'>
                <Popover placement='left-end' className='p-0'>
                  ...
                  <PopoverContent>
                    <Suspense fallback={<Spinner />}>
                      <ChromePicker
                        color={state.color}
                        onChange={(v) => setDesingerState({ key: 'color', v: v.hex })}
                      />
                    </Suspense>
                  </PopoverContent>
                </Popover>
                ...
                <Button
                  ...
                  onClick={() =>
                    setDesingerState({
                      key: 'textDecoration',
                      v: state.textDecoration === 'auto' ? 'underline' : 'auto',
                    })
                  }
                >
                  {underline}
